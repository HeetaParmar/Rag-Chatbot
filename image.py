        ...
        <ul class="list-group">
            {% for message in chat_history|reverse %}
                <li class="list-group-item chat-message">
                    ...
                    {% if message.audio %}
                        <button onclick="playAudio('{{ url_for('serve_audio', filename=message.audio) }}')">🔊</button>
                    {% endif %}
                </li>
            {% endfor %}
        </ul>
...
def ask_question():
    ...
    system_message = "System: Processing your request...\n"
    ...
    if is_greeting(question):
        ...
        if best_issue:
            ...
            answer = f"Troubleshooting Step 1: {decision_trees[best_issue]['steps'][0]}\nHave you performed this step? [Yes] [No]"
        else:
            similarities = [
            ...
            if not similarities or max(similarities) < 0.3:
                ...
                answer = most_relevant_chunk["text"]
    ...
    chat_history.append({"role": "assistant", "content": answer, "audio": audio_filename})
    session["chat_history"] = chat_history
    
    return render_template_string(chatbot_page, chat_history=chat_history, model=model, back_url="/")
